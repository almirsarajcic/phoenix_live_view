defmodule Phoenix.LiveViewTest.TreeDOM do
  ...
  @doc """
  Finds the `data-phx-resume` token from the root live view element (the element
  with `data-phx-main`). Returns nil when no `data-phx-resume` attribute is found.
  """
  @spec find_resume_token(tree :: term()) :: String.t() | nil
  def find_resume_token(tree) do
    # Look for data-phx-resume on the root live view element.
    ...
    # absent — we search for data-phx-resume directly instead.
    case filter(tree, fn node -> attribute(node, "data-phx-resume") end) do
      [{_, attributes, _} | _] ->
        case List.keyfind(attributes, "data-phx-resume", 0) do
          {_, token} -> token
          nil -> nil
        end
