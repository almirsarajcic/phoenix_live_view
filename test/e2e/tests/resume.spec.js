// StreamLive opts in to resume via `use Phoenix.LiveView, resume: true`.
// Connected-only work belongs in on_connect/1 (not behind `if connected?` in mount/3),
// so warm reconnect preserves stream state without re-running mount.
...
test("join-patch stream children are NOT detached on warm reconnect", async ({
  page,
}) => {
  ...
  await page.locator(`#${firstId}`).evaluate((el) => {
    el.setAttribute("data-resume-tagged", "true");
  });
  ...
  // reload drives a fresh dead render -> resume -> resumed WS join -> join-patch
  await page.reload();
  await syncLV(page);
...
test("stream items render correctly after warm reconnect", async ({ page }) => {
  await page.goto("/stream");
  await syncLV(page);
